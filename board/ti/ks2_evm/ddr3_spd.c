...
int ddrtimingcalculation(uint8_t buf[], struct ddr3_sodimm *spd,
			 struct ddr3_spd_cb *spd_cb)
{
	...
	/* SDRAM Cycle Time */
	spd->t_ck = buf[12] * mtb;
	spd_cb->ddrspdclock = ((1 / spd->t_ck) * 1000) * 2;
	...
	spd_cb->ddr_size_gbyte = (spd->rank * (256 << (buf[4] & 0xf))) / 1024;
...
void init_ddr3param(struct ddr3_spd_cb *spd_cb,
			   struct ddr3_sodimm *spd)
{
	...
	if (spd->rank == 2)
		spd_cb->emif_cfg.sdcfg |= DCE0_DEC1 << 3;
...
int get_dimm_params_from_spd(struct ddr3_spd_cb *spd_cb)
{
	int ret;
	int old_bus;
	struct ddr3_sodimm spd;
	...
	strncpy(spd_cb->dimm_name, (char *)&spd_params[0x80], 18);
