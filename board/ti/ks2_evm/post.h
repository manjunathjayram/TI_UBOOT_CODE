/*
 * KS2-EVM: Power On Self Test definitions
 *
 * Copyright (C) 2013 - 2014 Texas Instruments
 *
 ...
 ******************************************************************************/
#define POST_NAND_TEST_READ_ADDR	0x100000 /* NAND read offset address,
						    skip the U-boot partition */
 ...
 ******************************************************************************/
#define POST_EMAC_TEST_PKT_LENGTH	256	/* Ethernet packet payload size
						   in bytes */
...
#define PLATFORM_INFO_BOARD_NAME	"KS2_EVM"

extern int ddr_memory_test(u32 start_address, u32 end_address, int quick);

int keystone2_eth_phy_read(u_int8_t phy_addr, u_int8_t reg_num,
			 u_int16_t *data);
int keystone2_eth_phy_write(u_int8_t phy_addr, u_int8_t reg_num,
			  u_int16_t data);
