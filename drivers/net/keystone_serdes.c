...
static inline void serdes_rmw(u32 addr, u32 value, u32 mask)
{
...
static void serdes_cfg_setup(u32 base, struct serdes_cfg *cfg, u32 size)
{
	...
	for (i = 0; i < size; i++)
		serdes_rmw(base + cfg[i].ofs, cfg[i].val, cfg[i].mask);
}
...
static void serdes_comlane_setup(u32 base,
				 struct serdes_cfg *cfg_comlane, u32 size)
{
	serdes_cfg_setup(base, cfg_comlane, size);
}

static void serdes_lane_config(u32 base,
			       struct serdes_cfg *cfg_lane, u32 size, u32 lane)
{
	u32 i;

	for (i = 0; i < size; i++)
		serdes_rmw(base + cfg_lane[i].ofs + SERDES_LANE_REGS(lane),
...
static void serdes_cmu_comlane_enable(u32 base, struct serdes *serdes)
{
	...
	serdes_rmw(base + SERDES_CMU_REG_010(0), 0x0, SERDES_RESET);
	if (serdes->intf == SERDES_PHY_PCSR)
		serdes_rmw(base + SERDES_CMU_REG_010(1), 0x0, SERDES_RESET);

	/* Enable CMU and COMLANE */
	serdes_rmw(base + SERDES_CMU_REG_000(0), 0x03, 0x000000ff);
	if (serdes->intf == SERDES_PHY_PCSR)
		serdes_rmw(base + SERDES_CMU_REG_000(1), 0x03, 0x000000ff);

	serdes_rmw(base + SERDES_COMLANE_REG_000, 0x5f, 0x000000ff);
}

static void serdes_pll_enable(u32 base, struct serdes *serdes)
{
	writel(serdes_cfg_pll_enable[serdes->intf], base + SERDES_PLL_CTL_REG);
}

static void serdes_lane_reset(u32 base, u32 reset, u32 lane)
{
	if (reset)
		serdes_rmw(base + SERDES_LANE_REG_028(lane),
...
static void serdes_lane_enable(u32 base, struct serdes *serdes, u32 lane)
{
	...
	if (serdes->loopback)
		serdes_rmw(base + SERDES_LANE_REG_000(lane),
